...
modDatetime: 2026-07-06T10:00:00+09:00
...
## バイブコーディングツール「無料でどこまでできる？」2026年版まとめ

「まずは0円で試したい」という方が多いので、各ツールの無料枠を整理します。

- **v0 by Vercel**：1日7メッセージ（クレジットカード登録不要）。UI生成からプレビューURL発行まで無料枠内で完結します。LP1本分のヘッダー・コンテンツ・フッターなら7メッセージで十分に試せます。
- **Lovable**：試用枠あり（上限は頻繁に変更されるため公式サイトで確認推奨）。試用後は有料プランへ移行する流れです。
- **Bolt**：試用枠あり。StackBlitz の WebContainer 技術ベースのため、ブラウザだけで Node.js 環境を立ち上げて試せます。
- **Replit Agent**：試用枠あり。ただし本番公開（カスタムドメイン接続）には有料プランが必要です。
- **Figma Make**：Figma の既存プランに含まれています。個人用 Figma 無料プランでも利用できるため、Figma をすでに使っている人は追加費用ゼロで試せます。

**「どれを最初に使うか」に迷ったら**：クレジットカード不要で始められる **[v0](/tools/v0/)** が入口として最もハードルが低いです。1日7メッセージの枠で「本当に自分に向いているか」を確認してから有料判断すれば、無駄な出費を避けられます。
...
WordPress 系の本格運用なら **[mixhost](/tools/mixhost/)**（LiteSpeed 採用、月額 968円〜）。静的サイト or Vercel 系なら **Cloudflare Pages** が無料で十分機能します（[公式サイトを見る](/go/mixhost)）。WordPress ブログと組み合わせてアフィリエイトを運営するなら、国内2強の [ConoHa WING エックスサーバー 比較](/posts/xserver-vs-conoha-wing-2026/)（月額660円〜）も参考にしてください。
...
## バイブコーディングスキルで副業収入を得る3ステップ

「ツールで動くものが作れるようになったら、次は副業収入につなげたい」という方向けに、具体的なフローを整理します。

### Step 1：まず「公開実績」を1本作る

副業受注で最初に問われるのは実績です。v0 や Bolt の無料枠でLP1本、または社内ツールのプロトタイプを1本公開するだけで、「制作実績あり」で提案できるようになります。完成度よりも **公開 URL があること** が重要です。

### Step 2：LP制作・Web制作案件を受注する

LP制作・Webサイト制作・社内ツール開発は、ランサーズ・クラウドワークスに1〜5万円の案件として常時出ています。Vibe Coding で制作速度が上がると、1案件あたりの実質時給が大きく改善します。「コードを書けなくても動くものを納品できる」はクライアントの要件を満たします。

### Step 3：スキルを体系化して単価を上げる

「月5万円以上を安定させたい」段階では、HTML / CSS / JavaScript の基礎を体系的に整えることで受注単価が上がります。[忍者CODE](/go/ninja-code) はLP制作・Web受託案件の獲得に特化したプログラミングスクールで、Vibe Coding の成果物を実績として案件獲得につなげるサポートをしています（[忍者CODE レビュー詳細](/posts/ninja-code-review-2026/)）。
...
- [バイブコーディングツール比較2026：v0・Lovable・Bolt・Replit Agent を用途別に選ぶ](/posts/vibe-coding-tools-2026/) — 5ツールを料金・無料枠・向いている人で横並び比較
...
- [Catchy vs Transcope 徹底比較](/posts/catchy-vs-transcope-2026/) — 作ったプロダクトの「集客記事」をAIで量産するためのツール選び
