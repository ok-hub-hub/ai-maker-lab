...
記事1本 = 入力1,000＋出力2,000トークン、月利用量で計算した目安です。
...
AI Maker Lab の実運用比較（2026年7月）では次の差が出ています：

- **GPT-5.6 Luna**：レスポンスが速く入力コストが低いが、2,000字超の日本語SEO記事では段落間のトーン変化・文体ブレが出やすい。「下書きを出してから人が仕上げる」フロー前提なら許容範囲。完成品品質をそのまま出すには難しい。
- **Claude Sonnet 5**：1,000〜4,000字の完成品品質が安定している。キーワード埋め込みの自然さ・文体一貫性ともに高く、出力後の編集コストが少ない。月100本量産でも「書き直し0.5本分」の差がトータルコストに効いてくる。

### 結論：どちらを選ぶか

| 状況 | 推奨 |
|------|------|
| 下書き量産・人が仕上げる前提 | GPT-5.6 Luna（API コスト最安） |
| 完成品品質のSEO記事を量産 | Claude Sonnet 5（編集コスト込みのトータルが安い） |
| 2026年9月以降のコスパ見直し | 再比較推奨（Sonnet 5 が$3/$15 に上がるため差が縮む） |

月100本規模では差が小さいため、**まず Sonnet 5 で品質を確認し、量が増えてきたら Luna を下書き用に転用する**という段階的な使い方が現実的です。なお、API ではなく専用ツールで量産するなら、Catchy・Transcope がモデル選択を吸収してくれるため、この比較は不要になります。
...
Claude Sonnet 5 には**イントロ価格 $2/$10/MTok** が適用されていましたが、**2026年8月31日をもって終了**し、2026年9月1日以降は通常価格 $3/$15/MTok です。この価格シフトは「Sonnet 5 vs GPT-5.6 Luna どちらを選ぶか」の答えを変えます。

### 価格改定の前後で選択肢がどう変わるか

| 時期 | Claude Sonnet 5 | GPT-5.6 Luna | Sonnet 5 / Luna 差 | 記事1本（入力1k+出力2k tok）の差 |
|------|-----------------|--------------|-------------------|-------------------------------|
| 2026/8/31 に終了したイントロ価格 | $2.00/$10.00 | $1.00/$6.00 | **約1.7倍** | 約$0.009（≈1.4円） |
...
イントロ期間中は Sonnet 5 が GPT-5.6 Terra（$2.50/$15）より安く、Luna との差も小さいままでした。現行価格では **Sonnet 5 が Terra と同水準**に並ぶため、「低コスト量産だから Sonnet 5」という理由づけは成り立たなくなっています。
...
2026年8月31日に終了しており、2026年9月1日以降は通常価格（入力$3・出力$15/MTok）です（Anthropic ドキュメント 2026-08-28 再確認）。イントロ価格を前提に組んだコスト試算がある場合は、現行価格で引き直してください。
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "2026年8月31日に終了しており、2026年9月1日以降は通常価格（入力$3・出力$15/MTok）です（Anthropic ドキュメント 2026-08-28 再確認）。イントロ価格を前提に組んだコスト試算がある場合は、現行価格で引き直してください。"
      }
    },
    {
      "@type": "Question",
...
このページはサイトで最も読まれている入口ですが、ここから先は「何を決めたいか」で分かれます。

- **3モデルのコスパを横並びで見たい** → [claude opus 4.8 gpt5.5 の日本語ライティング比較](/posts/claude-sonnet5-vs-opus48-vs-gpt55-jp-writing-2026/)（Sonnet 5 を含む3モデル）
- **API直叩きをやめてツールに寄せたい（SEO記事の量産）** → [DeepEditor vs Transcope｜SEO用途で両方使った違い](/posts/deepeditor-vs-transcope-2026/)
- **広告コピー・SNS文の量産が主目的** → [Catchy と Transcope はどちらが向くか](/posts/catchy-vs-transcope-2026/)
- **書くより先にコードを書きたくなった** → [Cursor と Claude Code の比較](/posts/cursor-vs-claude-code-comparison/)
