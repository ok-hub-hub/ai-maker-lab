...
- 商用利用（**個人 Hobby プランは商用利用不可**、副業でアフィリ収益を伴う場合は Pro $20/月 が必要）
...
「**ドメイン＝[お名前.com](/go/onamae)、ホスティング＝[mixhost](/go/mixhost)、CMS＝WordPress、SEO＝Rank Math + Search Console**」は、日本の副業ブロガーの **黄金パターン**として10年以上定着しています。AI ツール紹介ブログ／レビューブログをこの構成で始めるのが、最も挫折率が低いと観察されています。
...
**Astro + Cloudflare Pages + お名前.com ドメイン** です。月額は **ドメイン代1,500円/年** のみ、Bandwidth 制限を気にする必要なし、Lighthouse スコアが出やすい。本サイトもこの構成です。
...
**個人 Hobby プランは「商用利用不可」**と Vercel 利用規約に明記されています。アフィリ収益・広告収益を伴う場合は **Pro $20/月** が必要です。これを避けたい場合は **Cloudflare Pages**（個人プランで商用OK）を選んでください。

### Q4. 速度（日本国内）はどれが速いですか？

実測では **Cloudflare Pages（東京拠点）と mixhost（LiteSpeed + 国内 DC）が拮抗**します。Vercel・Netlify は北米中心の拠点配置で、日本からのアクセスでわずかに遅れる傾向。ただし数十ミリ秒の差なので、SEO 順位への直接影響は限定的です。
